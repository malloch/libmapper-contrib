...
void QCPLayoutElement::update(UpdatePhase phase)
{
  if (phase == upMargins)
  {
    if (mAutoMargins != QCP::msNone)
    {
      ...
      for (auto const& side : QList<QCP::MarginSide>() << QCP::msLeft << QCP::msRight << QCP::msTop << QCP::msBottom)
      {
...
{
  for (auto const& el : elements(false))
  {
    if (!el->parentPlot())
...
QCPItemAnchor::~QCPItemAnchor()
{
  ...
  for (auto const& child : mChildrenX.toList())
  {
  ...
  for (auto const& child : mChildrenY.toList())
  {
...
{
  QList<QCPAbstractPlottable*> result;
  for (auto const& plottable : mPlottables)
  {
...
QList<QCPAxisRect*> QCustomPlot::axisRects() const
{
  ...
  {
    for (auto const& element : elementStack.pop()->elements(false))
    {
      if (element)
      {
...
QCPLayoutElement *QCustomPlot::layoutElementAt(const QPointF &pos) const
{
  ...
  while (searchSubElements && currentElement)
  {
    ...
    for (auto const& subElement : currentElement->elements(false))
    {
...
QList<QCPAxis*> QCustomPlot::selectedAxes() const
{
  ...
  for (auto const& rect : axisRects())
    allAxes << rect->axes();
  
  for (auto const& axis : allAxes)
  {
...
QList<QCPLegend*> QCustomPlot::selectedLegends() const
{
  ...
  while (!elementStack.isEmpty())
  {
    for (auto const& subElement : elementStack.pop()->elements(false))
    {
      if (subElement)
      {
...
void QCustomPlot::deselectAll()
{
  for (auto const& layer : mLayers)
  {
    for (auto const& layerable : layer->children())
...
void QCustomPlot::draw(QCPPainter *painter)
{
  ...
  for (auto const& el : findChildren<QCPLayoutElement*>())
  {
    painter->setBrush(Qt::NoBrush);
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  if (mType != type)
  {
    ...
    for (auto const& atype : QList<QCPAxis::AxisType>() << QCPAxis::atLeft << QCPAxis::atRight << QCPAxis::atBottom << QCPAxis::atTop)
    {
  ...
  mGradientImageInvalidated(true)
{
  ...
  for (auto const& type : QList<QCPAxis::AxisType>() << QCPAxis::atBottom << QCPAxis::atTop << QCPAxis::atLeft << QCPAxis::atRight)
  {
...
{
  for (auto& bars : mBars) // since foreach takes a copy, removing bars in the loop is okay
